...
class NavPosllhMsg:
	...
	def __str__(self):
		...
		horAcc = "Horizontal Accuracy Estimate: %.3f m" % (self.horAcc/1000.0)
		verAcc = "Vertical Accuracy Estimate: %.3f m" % (self.verAcc/1000.0)
...
while(1):
	...
	for byt in buffer:
		...
		if(ubl.mess_queue.empty() != True):
			msg = ubl.parse_ubx()
			if (msg != None):  r.publish('gps', json.dumps({
